    ...
    extra_args = [
        {
            ...
            "nargs": 1,
            "choices": ("list", "start", "stop", "pause", "status", "site_map", "zone_map"),
            "help": "List applicable devices, start/stop/pause mowing, show status or upload site/zone map",
        },
        {
        ...
        match app.args.command[0]:
            ...
            case "site_map":
                if (mower := app.device) is None:
                    return 1
                if not isinstance(mower, Gen2Mower):
                    print("Site map upload not supported")
                    return 1
                ...
                try:
                    ...
                    return 1
                request = mower.build_write_site_map_obj(svg_data)
                result = await app.send_request(request)
                if result is None or not result[0].success:
                    print("Failed to upload site  map")
                    if result is not None and isinstance(result[0], ErrorMessage):
                        print(f"Error: {result[0].error_message}")
                    return 1
