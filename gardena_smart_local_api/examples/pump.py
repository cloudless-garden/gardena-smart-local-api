...
from gardena_smart_local_api.devices import OperatingMode, Pump
    ...
    extra_args = [
        ...
        },
        {
            ...
            "help": (
                ...
                "For set-mode use: " + ", ".join(m.name.lower() for m in OperatingMode)
            ),
        },
    ]
    ...
    async with ExampleApp(COMPATIBLE, extra_args) as app:
        match app.args.command[0]:
            ...
            case "status":
                ...
                print(f"Valve error:         {pump.valve_error}")
            ...
            case "set-mode":
                if (pump := app.device) is None:
                    return 1
                assert isinstance(pump, COMPATIBLE)
                valid = ", ".join(m.name.lower() for m in OperatingMode)
                if app.args.value is None:
                    print(f"Missing value for set-mode. Available modes: {valid}")
                    return 1
                if app.args.value.upper() not in OperatingMode.__members__:
                    print(f"Invalid mode '{app.args.value}'. Valid modes: {valid}")
                    return 1

                mode = OperatingMode[app.args.value.upper()]
